...
public class LogitechDeviceProvider : AbstractRGBDeviceProvider
{
    ...
    protected override IEnumerable<IRGBDevice> LoadDevices()
    {
        ...
        IEnumerable<HIDDeviceDefinition<int, (LogitechDeviceType deviceType, int zoneOffset, int zones)>> wiredPerZoneDevices = PerZoneDeviceDefinitions.GetConnectedDevices().Select(x => x.definition);
        IEnumerable<HIDDeviceDefinition<int, (LogitechDeviceType deviceType, int zoneOffset, int zones)>> wirelessPerZoneDevices = PerZoneWirelessDeviceDefinitions.GetConnectedDevices();
        foreach (HIDDeviceDefinition<int, (LogitechDeviceType deviceType, int zoneOffset, int zones)> definition in wiredPerZoneDevices.Concat(wirelessPerZoneDevices)
                                                                                                                                       ...
                                                                                                                                       .Select(group => group.First()))
        {
            ...
            yield return new LogitechZoneRGBDevice(new LogitechRGBDeviceInfo(definition.DeviceType, definition.Name, LogitechDeviceCaps.DeviceRGB, definition.CustomData.zones, definition.CustomData.zoneOffset), updateQueue, definition.LedMapping);
        }
